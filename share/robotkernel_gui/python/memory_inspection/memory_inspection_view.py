...
class memory_inspection_view(helpers.service_provider_view, gui_utils.builder_ui):
    def __init__(self, parent, container, show_group):
        ui_filename = os.path.join(os.path.dirname(__file__), 'memory_inspection_view.ui')

        gui_utils.builder_ui.__init__(self, filename=ui_filename)
        container.pack_start(self.memory_inspection_box, True, True)
        self.memory_inspection_box.hide()
        show_group.add(self.memory_inspection_box)
        helpers.service_provider_view.__init__(self, parent.app, parent, show_group, self.memory_inspection_box, 'get_memory_areas')

        #self.kv_refresh_btn.connect("clicked", self.on_refresh)

        fd = gui_utils.get_monospace_font_description()
